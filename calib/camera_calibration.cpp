...
//for f in *.jpg; do     ffmpeg -i "$f" -vf scale="iw/2:ih/2" ../2k/"$f";  done
            printf("[%lu < %lu]", atImageList, imageList.size());
            cout << imageList[atImageList] << flush;
...
//TODO
    ...
    for(;;)
    {
        ...
        cout << "\t" << imagePoints.size() << ">=" << (size_t)s.nrFrames << ", calibration" << flush;
    ...
    if( s.inputType == Settings::IMAGE_LIST && s.showUndistorsed )
    {
        ...
        for(size_t i = 0; i < s.imageList.size(); i++ )
        {
            ...
            std::string bibo = s.imageList[i] + ".jpg";
            cout << endl << bibo << endl << flush;
            imwrite(bibo, rview);
